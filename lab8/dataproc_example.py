...
df = spark.read \
  .format("bigquery") \
  .load("de2023-397720.labdataset.retaildata")    # project_id.datatset.tablename. Use your project id
df.printSchema()
df.show(4)
...
bucket = "temp_de2023"  # use your bucket 
...
ordered_word_count.write.format('bigquery') \
  .option('table', 'de2023-397720.labdataset.wordcounts') \
  .mode("append") \
  .save()
